...
int main(int argc, char *argv[]){
		...
		if (argc < 2) {
		    cerr << "Usage: count filename.txt" <<endl;
		}
		...
		while(inFile && inFile.peek() != -1){
			...
			if(DLLMan.checkForOper(wholeLine)){
				...
				int term1Length = opIndex/digitsPerNode + 1;
				int term2Length = (dummy.length() - opIndex)/digitsPerNode + 1;
						
				node*T1Nodes[term1Length];
				node*T2Nodes[term1Length];
				
				DLLMan.createTNodes(T1Nodes, term1Length);
				DLLMan.createTNodes(T2Nodes, term2Length);
				DLLMan.createDLL(term1, T1Nodes, term1Length);
				DLLMan.createDLL(term2, T2Nodes, term2Length);
